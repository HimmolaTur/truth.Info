...
export default async function AdminNewsPage({
  ...
  await requireStaffPermission("content.news.read");

  const locale = await getLocale();
  const staffPerms = await getStaffPermissions();
  const canCreate = staffPerms.includes("content.news.create");
  const canUpdate = staffPerms.includes("content.news.update");
  ...
  let countQuery = "SELECT COUNT(*)::int AS count FROM news";
  let listQuery = `SELECT id, title, created_at,
       LEFT(COALESCE(content::text, ''), 500) AS content
     FROM news`;
  const params: unknown[] = [];

  if (q) {
    countQuery += " WHERE title ILIKE $1 OR content ILIKE $1";
    listQuery += " WHERE title ILIKE $1 OR content ILIKE $1";
    params.push(`%${q}%`);
  }
  ...
  try {
    ...
    const newsResult = await queryWithTimeout(listQuery, dataParams);
  ...
  return (
    <div className="max-w-5xl mx-auto w-full py-12 px-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        {canCreate ? (
          <Link href="/admin/news/new" className="inline-flex justify-center bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-700 transition">
            {t("createNews")}
          </Link>
        ) : null}
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <LocaleGetSearchForm basePath="/admin/news" className="flex-1 flex flex-col sm:flex-row gap-3">
      ...
      <div className="space-y-4">
        {news.length === 0 ? (
          <div className="text-center text-gray-500 py-12 border rounded-xl bg-white dark:bg-neutral-900 shadow-sm">
            {q ? tNews("noResults") : t("adminNewsEmpty")}
          </div>
        ) : (
          news.map((item) => (
            <div
            ...
            >
              <div className="min-w-0 flex-1">
                <h3 className="font-bold text-lg break-words">{item.title}</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 line-clamp-3">{textPreview(item.content)}</p>
                <div className="text-xs text-gray-400 mt-2">{new Date(item.created_at).toLocaleString(locale)}</div>
              </div>

              <div className="flex flex-row sm:flex-col gap-3 sm:gap-2 sm:items-end shrink-0">
                {canUpdate ? (
                  <Link href={`/admin/news/edit/${item.id}`} className="text-sm text-blue-600 font-medium">
                    {t("edit")}
                  </Link>
                ) : null}
              </div>
