        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white dark:bg-neutral-900 rounded-2xl border shadow-sm p-6 text-center">
            ...
            <div className="inline-flex items-center justify-center gap-1.5 bg-gray-100 dark:bg-neutral-800 px-3 py-1 rounded-full text-sm text-gray-600 dark:text-gray-300 mb-4 w-full">
              ...
              <span>Шифр: <span className="font-mono font-bold">{user.display_name}</span></span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 border-y dark:border-neutral-800 py-4 mb-4">
              <div>
                ...
                <div className="text-xs text-gray-500 uppercase tracking-wider font-bold mt-1">Карма</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-gray-900 dark:text-white">{topics.length + comments.length}</div>
                <div className="text-xs text-gray-500 uppercase tracking-wider font-bold mt-1">Публикаций</div>
              </div>
            </div>

            <div className="text-sm text-gray-500 flex items-center justify-center gap-2 mb-6">
              <Calendar className="w-4 h-4" />
              На форуме с {new Date(user.created_at).toLocaleDateString('ru-RU')}
            </div>

            <form action="/api/auth/logout" method="POST">
              <button type="submit" className="w-full inline-flex items-center justify-center gap-2 bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/40 px-4 py-2.5 rounded-xl font-bold transition border border-red-100 dark:border-red-900/30">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-2xl border shadow-sm overflow-hidden">
            <div className="p-5 border-b dark:border-neutral-800 bg-gray-50 dark:bg-neutral-800/50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold">Мои уведомления</h2>
              </div>
              {notifications.some(n => !n.is_read) && (
                <span className="bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full">Новые</span>
              )}
            </div>
            <div className="divide-y dark:divide-neutral-800 max-h-[300px] overflow-y-auto">
              {notifications.length === 0 ? (
                <div className="p-8 text-center text-gray-500">У вас пока нет уведомлений.</div>
              ) : (
                notifications.map(n => (
                  <Link key={n.id} href={`/forum/${n.topic_id}${n.topic_title ? '-' + slugify(n.topic_title) : ''}`} className={`block p-4 hover:bg-gray-50 dark:hover:bg-neutral-800/50 transition ${!n.is_read ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}>
                    ...
                    <div className="text-xs text-gray-500 mt-1">{new Date(n.created_at).toLocaleString('ru-RU')}</div>
                  </Link>
                ))
              )}
            </div>
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-2xl border shadow-sm overflow-hidden">
            <div className="p-5 border-b dark:border-neutral-800 bg-gray-50 dark:bg-neutral-800/50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold">Мои темы ({topics.length})</h2>
              </div>
            ...
            <div className="divide-y dark:divide-neutral-800 max-h-[400px] overflow-y-auto">
              {topics.length === 0 ? (
                <div className="p-8 text-center text-gray-500">Вы еще не создали ни одной темы.</div>
              ) : (
                topics.map(topic => (
          ...
          <div className="bg-white dark:bg-neutral-900 rounded-2xl border shadow-sm overflow-hidden">
            <div className="p-5 border-b dark:border-neutral-800 bg-gray-50 dark:bg-neutral-800/50 flex items-center gap-3">
              ...
              <h2 className="text-lg font-bold">Мои ответы ({comments.length})</h2>
            </div>
            <div className="divide-y dark:divide-neutral-800 max-h-[400px] overflow-y-auto">
              {comments.length === 0 ? (
                <div className="p-8 text-center text-gray-500">Вы еще не оставляли комментариев.</div>
              ) : (
                comments.map(comment => (
                  <Link href={`/forum/${comment.topic_id}-${slugify(comment.topic_title)}`} key={comment.id} className="block p-4 hover:bg-gray-50 dark:hover:bg-neutral-800/50 transition">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
                      <span>{new Date(comment.created_at).toLocaleDateString('ru-RU', { hour: '2-digit', minute: '2-digit' })}</span>
                      <span className="flex items-center gap-1"><ThumbsUp className="w-3 h-3" /> {comment.likes || 0}</span>
                      {comment.is_best_answer && <span className="text-green-600 font-bold">Лучший ответ</span>}
                    </div>
                  </Link>
                ))
              )}
